...
public class CommandHandler {
    ...
    public CommandHandler(Logger logger) {
        this.commandList = Arrays.asList(new HelpCommand(), new LogLevelCommand(),
                new ConfigCommand(), new ServerTypeCommand(), new ListCommand(), new StopCommand(), new BackupCommand(),
    ...
    public void tryToExecuteCommand(String[] args) {
        ...
        String cmd = args[0];
        log.debug("Console command: {}", Arrays.toString(args));
        ...
        log.trace("Command found to console input: {}", command);
        if (command == null) {
